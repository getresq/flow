...
export function EventDetailContent({ entry, hasJourney, onOpenRun }: EventDetailContentProps) {
  const displayMessage = entry.message;
  const errorMessage =
    typeof entry.attributes?.error_message === 'string'
      ? entry.attributes.error_message
      : undefined;
    ...
    <ScrollArea className="h-full">
      <div className="space-y-4 px-4 py-3">
        ...
        <details className="min-w-0 overflow-hidden rounded-lg border border-[var(--border-default)]">
          <summary className="cursor-pointer p-3 text-xs text-[var(--text-muted)]">
            Raw telemetry
          </summary>
